...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="<c:url value='${fns:getAdminPath()}/cms/article/appform?id=${article.id}'><c:param name='category.name' value='${article.category.name}'/></c:url>">文章<shiro:hasPermission name="cms:article:edit">${not empty article.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="cms:article:edit">查看</shiro:lacksPermission></a></li>
		<c:if test="${typeApp eq 'zjjs' || typeApp eq 'tsg' || typeApp eq 'xyjs'}">
			<li ><a href="${ctx}/cms/cmsAppNormal?typeApp=${typeApp}">信息列表</a></li>
			<li><a href="${ctx}/cms/cmsAppNormal/form?typeApp=${typeApp}">信息添加</a></li>
		</c:if>
		<c:if test="${typeApp eq 'gjc'}">
			<li><a href="${ctx}/cms/cmsBus/">公交线路列表</a></li>
			<li><a href="${ctx}/cms/cmsBus/form">公交线路添加</a></li>
		</c:if>
		<c:if test="${typeApp eq 'tcc'}">
			<li><a href="${ctx}/cms/cmsParkingLot/">停车场列表</a></li>
			<li><a href="${ctx}/cms/cmsParkingLot/form">停车场添加</a></li>
		</c:if>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="article" action="${ctx}/cms/article/appsave" method="post" class="form-horizontal">
		<form:hidden path="id"/>
